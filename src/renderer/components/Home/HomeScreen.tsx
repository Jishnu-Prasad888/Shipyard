...
import {
  LayoutDashboard, FolderKanban, KanbanSquare, Plus, FolderOpen,
...
interface Workspace { id: string; name: string; color: string; parentWorkspaceId?: string | null }
interface Project { id: string; name: string; color: string; workspaceId?: string | null; description?: string; tags?: string; createdAt: number }
interface BoardSummary { id: string; name: string; projectId: string; color?: string; createdAt: number; description?: string }

export const HomeScreen: React.FC<HomeScreenProps> = ({ onSelectProject, onSelectBoard, dataVersion }) => {
  ...
  const boardsForProject = (projectId: string) => boards.filter(board => board.projectId === projectId)

  const filteredProjects = projects.filter(project => {
    const matchesWorkspace =
      activeWorkspace === 'all' ||
      (activeWorkspace === 'unassigned' ? !project.workspaceId : project.workspaceId === activeWorkspace)
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          ...
          <div className="space-y-2 px-2 py-3">
            ...
            {workspaces.map(workspace => {
              const workspaceProjectCount = projects.filter(project => project.workspaceId === workspace.id).length
              ...
              >
                ...
                />
                <span className="text-xs font-black uppercase tracking-wide flex-1 truncate">{workspace.name}</span>
            ...
            {unassignedCount > 0 && (
              <button
                ...
                style={{
                  border: '2px solid',
                  borderColor: activeWorkspace === 'unassigned' ? 'var(--color-muted)' : 'transparent',
                  background: activeWorkspace === 'unassigned' ? 'rgba(54,80,107,0.16)' : 'transparent',
                  color: activeWorkspace === 'unassigned' ? 'var(--color-text)' : 'var(--color-muted)',
                  boxShadow: activeWorkspace === 'unassigned' ? 'var(--shadow-brutal-sm)' : 'none'
                }}
              >
                <FolderKanban className="w-4 h-4 shrink-0 opacity-70" />
                <span className="text-xs font-black uppercase tracking-wide flex-1">Unassigned Projects</span>
        ...
        <div className="flex-1 overflow-auto px-6 py-5 space-y-8">
          ...
          {filteredProjects.length === 0 ? (
            ...
            >
              <FolderOpen className="w-12 h-12 opacity-20" style={{ color: 'var(--color-primary)' }} />
              <div className="text-center">
                ...
                {!filter && (
                  <button onClick={() => setShowCreateProject(true)} className="btn-primary mt-4 text-xs uppercase tracking-wider">
                    <Plus className="w-4 h-4 stroke-[3px]" />
                    Create Project
                  </button>
                )}
            ...
            <div className="space-y-6">
              {filteredProjects.map(project => {
                const projectBoards = boardsForProject(project.id).filter(board =>
                  !filter || board.name.toLowerCase().includes(filter.toLowerCase())
                )
                  ...
                  <section key={project.id}>
                    ...
                    >
                      <div
                        ...
                        style={{ background: projectColor, boxShadow: `0 12px 28px ${projectColor}33` }}
                      />
                      ...
                      {workspace && (
                        <span
                          ...
                          style={{ borderColor: workspace.color || projectColor, color: workspace.color || projectColor }}
                        >
                          {workspace.name}
                        </span>
                      )}
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                        {projectBoards.map(board => (
                          ...
                          >
                            ...
                            />
                            <KanbanSquare className="w-4 h-4 mb-2" style={{ color: projectColor }} />
                            ...
                            <div className="flex items-center justify-between mt-2">
                              <LayoutDashboard className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: projectColor }} />
                              <ChevronRight className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: projectColor }} />
                            </div>
                          </button>
                        ))}
                      </div>
